...
def populate_order():
    # the below function gives us access to the database
    with app.app_context():

    #enumerate is a function that give u the index and position of an item
        for index, books in enumerate(BIBLE_BOOKS):
            #gets the id from the dictionary
            book_id = books["id"]
            #querying
            book = Bible_books.query.filter_by(id=book_id).first()
            if not book:
                print ("Books not found")
            else:
                bible_order = index +1
                book.book_order = bible_order
                print(f"Updated {book.name} with order {bible_order}")

        db.session.commit()
        print("\n All book orders updated successfully!")
